...
void PORT1_IRQHandler(void){
	...
	if ((P1->IFG & (uint8_t)(1<<1)) != 0) {
		...
		//Turn off Reset mode if switch is pressed in reset mode
		if (reset>0){
			reset = 0;
			P1->IFG &= (uint8_t)~(1<<1);
		}
		
		//same as bonus 1 and 2 (see bonus 1 and 2 for comments)
		if (cycle<=4 && cycle>=0){
		 cycle = 3 ;
		P1->IFG &= (uint8_t)~(1<<1);
		}
...
void TA0_N_IRQHandler(void){
	...
	//reset mode is on, only cycle between off and red
	if (reset > 0 ){
			reset++;
			P2->OUT &= ~(RGB_LED_MASK);
			if (reset%2 == 1){
