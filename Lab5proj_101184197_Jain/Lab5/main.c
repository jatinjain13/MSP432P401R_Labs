...
static uint8_t count = 2; //To select which color is shown on LED
static uint8_t cycle = 0; // cycles between 1 - 9 seconds
...
void PORT1_IRQHandler(void){
	...
	static volatile uint16_t i = 5000;
...
void TA0_N_IRQHandler(void){
	...
	//RED
		...
		//Clear LED then change to color needed
		P2->OUT &= ~(RGB_LED_MASK);
		P2->OUT |= count;
	}
